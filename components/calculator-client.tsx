...
function IngredientEditor({ value, onChange, schema }: { value: IngredientRow[]; onChange: (value: IngredientRow[]) => void; schema?: { first: string; amount: string; unit: string; name: string } }) {
  const labels = schema ?? { first: "Ingredients", amount: "Amount", unit: "Unit", name: "Ingredient" };
  return <fieldset className="cf-ingredients"><legend>{labels.first}</legend>{value.map((row, index) => <div className="cf-ingredient-row" key={`${index}-${row.ingredient}`}><input aria-label={`${labels.amount} amount`} placeholder="1/2" value={row.amount} onChange={(event) => onChange(value.map((item, i) => i === index ? { ...item, amount: event.target.value } : item))} /><input aria-label={`${labels.unit} unit`} placeholder={labels.unit === "Ingredient" ? "Type" : "cup"} value={row.unit} onChange={(event) => onChange(value.map((item, i) => i === index ? { ...item, unit: event.target.value } : item))} /><input aria-label={`${labels.name} name`} placeholder={labels.name} value={row.ingredient} onChange={(event) => onChange(value.map((item, i) => i === index ? { ...item, ingredient: event.target.value } : item))} /></div>)}<button className="cf-add-row" type="button" onClick={() => onChange([...value, { ingredient: "", amount: "", unit: "" }])}>+ Add {labels.name.toLowerCase()}</button></fieldset>;
}
