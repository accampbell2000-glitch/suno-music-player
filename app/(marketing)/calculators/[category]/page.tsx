...
export async function generateMetadata({ params }: { params: Promise<{ category: string }> }): Promise<Metadata> { const { category: slug } = await params; const category = categoryBySlug[slug]; return category ? { title: `${category.name} Calculators | CalcForge`, description: `${category.description} Browse CalcForge tools in ${category.name}.` } : {}; }
